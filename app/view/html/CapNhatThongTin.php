        ...
        <main>
            ...
            if ($_SERVER['REQUEST_METHOD'] === "POST"){
                if (isset($_POST['update'])){
                    ...
                    $update = $db->handle("UPDATE users SET username = '$up_username', 
                                                    email = '$up_email', password = '$up_pass' WHERE id_users = '$id'");
                }
            }
            $result = $db->getData("SELECT * FROM users u JOIN roles r ON u.role = r.id_role WHERE id_users = '$id'");
            if(!empty($result)){
            $data = $result[0];
            echo '<div id="updateUsers" class="update-users">';
                echo '<div class="update-users-content">';
                    ...
                    echo '<form method="POST" class="update-users-data">';
                            ...
                            echo '<div class="update-data">
                            <input type="text" name="old_username" value="'.$data['username'].'"></div>';
                            echo '<div class="update-data">
                            <input type="email" name="old_email" value="'.$data['email'].'"></div>';
                            echo '<div class="update-data">
                            <input type="text" name="old_pass" value="'.$data['password'].'"></div>';
                            echo '<div class="update-data">';
                            ...
                            echo '<div class="update-users-submit">';
                                echo '<li>';
                                    echo '<a href="../../view/html/Quanlycauhinh.php" class="first-child">Thoát</a>';
                                echo '</li>';
                                echo '<li>';
                                    echo '<a href="../../view/html/Quanlycauhinh.php"><input type="submit" name="update" class="last-child" value ="Cập nhật"></a>';
                                echo '</li>';
                            echo '</div>';
                    echo '</form>';
                echo '</div>';
